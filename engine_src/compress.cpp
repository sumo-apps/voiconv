...
MemBuffer *TEN::lzma_compress(MemBuffer *from, bool deleteFrom)
{
    ...
    // unsigned char *compressed_data = lzmaCompress( from->data, (uint32_t)from->size(), &compressed_size)
    //  set up properties
    ...
    if (lzmaStatus == SZ_OK)
    {
        ...
        /*
        size_t outputSize = outputSize64 + 13;
        // tricky: we have to generate the LZMA header
        // 5 bytes properties + 8 byte uncompressed size
        // result = std::make_unique<uint8_t[]>(outputSize64 + 13);
        Byte *result = (Byte *)malloc(outputSize);
        ...
        memcpy(result + 13, outputData, outputSize64);
        free(outputData);
        return new MemBuffer(result, outputSize, true);
        */
    }
...
MemBuffer *TEN::lzma_decompress(MemBuffer *from, bool deleteFrom)
{
    ...
    /*
    for (int i = 0; i < 8; i++) {
        printf("SizeChar %d: %d (hex: %x) \n", i, input[5+i], input[5+i]);
        size |= (((UInt64)input[5 + i]) << (i * 8));
    }
    */
    ...
    Byte *input = (Byte *)from->data;
    SizeT inputSize = (SizeT)from->size();
    // extract the size from the header
    UInt64 size = 0;
    for (int i = 0; i < 8; i++)
        size |= (input[5 + i] << (i * 8));
